...
This project provides a Docker container for [File Centipede](https://filecxx.com).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of File Centipede.
...
| 10111 | TCP | Optional | Port for File Centipede service, used by the web browser extension. |
| 17654 | TCP | Optional | Port for incoming BitTorrent peer connections. Can be changed from File Centipede. Your router must forward this port to the container. |
| 17654 | UDP | Optional | Port for incoming BitTorrent DHT messages. Can be changed from File Centipede. Your router must forward this port to the container. |
...
```shell
docker stop file-centipede
```

  2. Remove the container:
```shell
docker rm file-centipede
```
...
services:
  file-centipede:
    image: jlesage/file-centipede
    ports:
      - "5800:5800"
      - "10111:10111"
    volumes:
      - "/docker/appdata/file-centipede:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-file-centipede/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/file-centipede/tags
...
```shell
docker pull jlesage/file-centipede
```
...
For Synology NAS users, follow these steps to update a container image:
  ...
  3.  In the search bar, type the name of the container (`jlesage/file-centipede`).
  4.  Select the image, click *Download*, and choose the `latest` tag.
  5.  Wait for the download to complete. A notification will appear once done.
  6.  Click *Container* in the left pane.
  7.  Select your File Centipede container.
...
container, it would proxy all HTTP requests for `file-centipede.domain.tld` to
...
upstream docker-file-centipede {
...
server {
	...
	server_name file-centipede.domain.tld;

	location / {
	        proxy_pass http://docker-file-centipede;
	}
...
server {
	...
	location = /file-centipede {return 301 $scheme://$http_host/file-centipede/;}
	location /file-centipede/ {
		proxy_pass http://docker-file-centipede/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /file-centipede/websockify {
			proxy_pass http://docker-file-centipede/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /file-centipede/websockify-audio {
			proxy_pass http://docker-file-centipede/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of File Centipede integrated into Docker image. |
...
Any desktop notification generated by File Centipede is forwarded to the
...
File Centipede requires an activation code beginning two days after
installation.
